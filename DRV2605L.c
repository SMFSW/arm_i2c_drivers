...
FctERR NONNULL__ DRV2605L_Write(const uint8_t * data, const uint16_t addr, const uint16_t nb)
{
	...
	if (!I2C_is_enabled(&DRV2605L_hal))					{ err = ERROR_DISABLED; }	// Peripheral disabled
	if ((addr + nb) > DRV__LRA_RESONANCE_PERIOD + 1U)	{ err = ERROR_OVERFLOW; }	// More bytes than registers
	...
	err = HALERRtoFCTERR(DRV2605L_hal.status);
