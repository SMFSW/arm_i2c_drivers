...
/****************************************************************/
#if (defined(I2C_PCA9624) || defined(PCA9685) || defined(I2C_PCA9952)) && !defined(NO_WARN_I2C_DRIVERS)
...
FctERR NONNULL__ PCA9956_Init(const uint8_t idx, I2C_HandleTypeDef * const hi2c, const uint16_t devAddress)
{
	...
	I2C_slave_init(&PCA9956_hal[idx], hi2c, devAddress, PCA9956_hal[idx].cfg.timeout);

	err = PCA9956_Init_Sequence(&PCA9956[idx]);

	if (err != ERROR_OK)	{ I2C_set_enable(&PCA9956_hal[idx], false); }
...
FctERR NONNULL__ PCA9956_Write(I2C_slave_t * const pSlave, const uint8_t * data, const uint16_t addr, const uint16_t nb)
{
	...
	if ((addr + nb) > PCA9956__IREFALL + 1U)	{ err = ERROR_OVERFLOW; }	// More bytes than registers
...
FctERR NONNULL__ PCA9956_Read(I2C_slave_t * const pSlave, uint8_t * data, const uint16_t addr, const uint16_t nb)
{
	...
	if (!I2C_is_enabled(pSlave))				{ err = ERROR_DISABLED; }	// Peripheral disabled
	if ((addr + nb) > PCA9956__EFLAG5 + 1U)		{ err = ERROR_OVERFLOW; }	// More bytes than registers
	if (err != ERROR_OK)						{ goto ret; }
