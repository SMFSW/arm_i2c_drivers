...
{
	const int16_t val = (int16_t) LSHIFT(pixel, 4U);	// VAL is signed 12b, result done by shifting to 16b signed
	return (float) (val * 0.015625f);					// and multiplying by granularity (0.25°C) divided by 16
}
...
FctERR NONNULL__ AMG88_Get_Pixel_Raw(AMG88_t * const pCpnt, uint16_t * raw, const uint8_t pixel)
{
	...
	if (pixel >= 64U)	{ err = ERROR_VALUE; }
	else				{ err = AMG88_Read_Word(pCpnt->cfg.slave_inst, raw, AMG88__T01L + (2U * pixel)); }
...
__WEAK bool NONNULL__ AMG88_INT_GPIO_Get(const AMG88_t * const pCpnt) {
