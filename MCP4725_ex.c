...
FctERR NONNULL__ MCP4725_Write_Command(MCP4725_t * const pCpnt, const uint16_t val)
{
	...
	if (pCpnt->cfg.Mode > MCP4725__WRITE_DAC_EEP)	{ err = ERROR_CMD; }	// Unknown command
	...
	if (pCpnt->cfg.Mode == MCP4725__FAST_MODE)
	{
		...
		CMD[1] = (uint8_t) val;
		err = MCP4725_Write(pCpnt->cfg.slave_inst, CMD, 2U);
	}
	else
	{
		...
		err = MCP4725_Write(pCpnt->cfg.slave_inst, CMD, 3U);
	}
