...
{
	const PCA962x_reg_inc inc_mode = PCA9952[pSlave - PCA9952_hal].cfg.auto_inc & 0x80;

	if (!I2C_is_enabled(pSlave))				{ return ERROR_DISABLED; }	// Peripheral disabled
	if ((addr + nb) > PCA9952__IREFALL + 1)		{ return ERROR_OVERFLOW; }	// More bytes than registers
...
FctERR NONNULL__ PCA9952_Read(I2C_slave_t * const pSlave, uint8_t * data, const uint16_t addr, const uint16_t nb)
{
	...
	if (!I2C_is_enabled(pSlave))				{ return ERROR_DISABLED; }	// Peripheral disabled
	if ((addr + nb) > PCA9952__EFLAG1 + 1)		{ return ERROR_OVERFLOW; }	// More bytes than registers
