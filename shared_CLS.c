...
const double CLS_RGB2XYZ_Default[3][3] = {
	{ -0.14282, 1.54924, -0.95641 },
	{ -0.32466, 1.57837, -0.73191 },
	{ -0.68202, 0.77073, 0.56332 } };		//!!< This matrix is the result of a transformation composed of correlations at different light sources (based on TAOS DN25 application note)
...
FctERR NONNULL__ CLS_get_chromacity(float xy[2], uint32_t * illum, const double mat[3][3], const uint16_t r, const uint16_t g, const uint16_t b)
{
	...
	Y = (mat[1][0] * r) + (mat[1][1] * g) + (mat[1][2] * b);		// Note: Y = Illuminance (lux)
